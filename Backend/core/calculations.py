...
class Vector:
    ...
    def find_dot(self, other_vector):
        ...
        steps = (
            f" Step 1: Multiply The Corresponding Components: \n"
            f" ({self.x} * {other_vector.x}) = {self.x * other_vector.x} \n"
            f" ({self.y} * {other_vector.y}) = {self.y * other_vector.y} \n"
            f" ({self.z} * {other_vector.z}) = {self.z * other_vector.z} \n"
            f" Step 2: Take The Sum of The Components: \n"
            f" {self.x * other_vector.x} + {self.y * other_vector.y} + {self.z * other_vector.z} = {result}"
        )
        return result, steps
    ...
    def cross_prod(self, other_vector):
        ...
        result = Vector(x, y, z)
        steps = (
            f" Step 1: Calculate Each Component: \n"
            f" x = ({self.y} * {other_vector.z}) - ({self.z} * {other_vector.y} = {x} \n"
            f" y = -(({self.x} * {other_vector.z}) - ({self.z} * {other_vector.x})) = {y} \n"
            f" z = ({self.x} * {other_vector.y}) - ({self.y} * {other_vector.x}) = {z} \n"
            f" Step 2: Combine The Components Into The Resulting Vector: \n"
            f" <{x}, {y}, {z}>"
        )
    ...
    def magnitude(self):
        ...
        steps = (
            ...
            f" {self.y}^2 = {self.y**2} \n"
            f" {self.z}^2 = {self.z**2} \n"
            f" Step 2: Take The Sum of All Components: \n"
            f" {self.x**2} + {self.y**2} + {self.z**2} = {self.x**2 + self.y**2 + self.z**2}\n"
            f" Step 3: Take the square root of the sum:\n"
            f" sqrt({self.x**2 + self.y**2 + self.z**2} = {pretty(magnitude)})"

        )

        return pretty(magnitude), steps

    # With A Given Vector, Find The Unit Vector And Return It In The Format '< x / magnitude, y / magnitude, z / magnitude
    def unit_vector(self):
        magnitude, mag_steps = self.magnitude()
        steps = (
            f" {mag_steps} \n"
            f" Step 4: Divide Each Component By The Magnitude: \n"
            f" x = {self.x} / {magnitude} \n "
            f" y = {self.y} / {magnitude} \n"
            f" z = {self.z} / {magnitude} \n"
            f" Step 5: Combine All Components Into Resulting Vector: \n"
            f" <{self.x}/{magnitude}, {self.y}/{magnitude}, {self.z}/{magnitude}>"
        )
        return Vector(f'{self.x}/{magnitude}', f'{self.y}/{magnitude}', f'{self.z}/{magnitude}'), steps
...
class Expression:
    ...
    def take_deriv(self, variable):
        ...
        deriv = expr.diff(x)
        steps = (
            f" Step 1: Identify The Variable We Are Differentiating In Respect to: {x} \n"
            f" Step 2: Differentiate The Expression {self.expression} With Respect To {variable} \n"
            f" Result: {pretty(deriv)}"
        )
        return pretty(deriv), steps
    ...
    def take_integral(self, variable, limits=None):
        ...
        if limits:
            ...
            steps = (
                f" Step 1: Identify The Variable For Integration: {variable} \n"
                f" Step 2: Identify The Limits of Integration: {lower_bound} To {upper_bound} \n"
        ...
        # If No Limits, take the expression and integrate with respect to x. (Indefinite Integral) 
            ...
            steps = (
                f" Step 1: Identify the variable for integration: {variable}\n"
                f" Step 2: Integrate the expression {self.expression} with respect to {variable}\n"
                f" Result: {pretty(integral)}"
            )
        return pretty(integral), steps


# --- Everything below this comment is previous functionality that was used to take in user data/input when this was solely a python script and had no frontend. --- #
